...
public class MatterFuserContainer extends Container {
    private TileEntity tileEntity;
    private PlayerEntity playerEntity;
    private IItemHandler playerInventory;

    public MatterFuserContainer(int windowID, World world, BlockPos blockPos, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        super(ModContainers.MATTER_FUSER_CONTAINER.get(), windowID);
        ...
        if(tileEntity != null) {
            tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
                ...
                addSlot(new SlotItemHandler(h, 1, 74, 11));
            });
        }
    ...
    public boolean hasItemInside() {
        return ((MatterFuserTile) tileEntity).hasItemInside();
    }

    public float getPercentageCompleted() {
        return tileEntity.getCapability(CapabilityMachine.MACHINE).map(IMachineHandler::getPercentageCompleted).orElse(0.0f);
    }
    ...
    public boolean canInteractWith(PlayerEntity playerIn) {
        return isWithinUsableDistance(IWorldPosCallable.of(tileEntity.getWorld(), tileEntity.getPos()), playerEntity, ModBlocks.MATTER_FUSER.get());
    }
